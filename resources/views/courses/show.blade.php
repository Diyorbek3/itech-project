        ...
        <div class="container position-relative">
            <div class="row align-items-center py-5">
                <div class="col-lg-7 text-white">
                    ...
                    <p class="hero-subtitle">{{ $course->short_description ?? (app()->getLocale() == 'uz' ? 'Professional kurs bilan kelajagingizni quring' : (app()->getLocale() == 'ru' ? 'Постройте свое будущее с профессиональным курсом' : 'Build your future with professional course')) }}</p>
            ...
            <div class="col-lg-8">
                ...
                <div class="stats-grid mb-5">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-content">
                            <h4>{{ $course->language ?? (app()->getLocale() == 'uz' ? 'O\'zbek' : (app()->getLocale() == 'ru' ? 'Узбекский' : 'Uzbek')) }}</h4>
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-content">
                            <h4>{{ $course->schedule ?? (app()->getLocale() == 'uz' ? '3 kun' : (app()->getLocale() == 'ru' ? '3 дня' : '3 days')) }}</h4>
                ...
                <div class="content-card mb-4">
                    <div class="card-header-custom">
                        ...
                        <h3>{{ app()->getLocale() == 'uz' ? 'Kurs haqida' : (app()->getLocale() == 'ru' ? 'О курсе' : 'About course') }}</h3>
                    </div>
                    <p class="card-text">
                ...
                <div class="content-card mb-4">
                    <div class="card-header-custom">
                        ...
                        <h3>{{ app()->getLocale() == 'uz' ? 'O\'quv dasturi' : (app()->getLocale() == 'ru' ? 'Учебная программа' : 'Curriculum') }}</h3>
                    </div>
                ...
                <div class="content-card mb-4">
                    <div class="card-header-custom">
                        ...
                        <h3>{{ app()->getLocale() == 'uz' ? 'Kimlar uchun?' : (app()->getLocale() == 'ru' ? 'Для кого?' : 'For whom?') }}</h3>
                    </div>
                ...
                <div class="content-card">
                    <div class="card-header-custom">
                        ...
                        <h3>{{ app()->getLocale() == 'uz' ? 'O\'qituvchilar' : (app()->getLocale() == 'ru' ? 'Преподаватели' : 'Teachers') }}</h3>
                    </div>
                    <div class="teacher-card">
                        ...
                        <div class="teacher-info">
                            ...
                            <div class="teacher-rating">
                                ...
                                <span class="rating-text">4.9 - {{ app()->getLocale() == 'uz' ? 'Mutaxassis o\'qituvchi' : (app()->getLocale() == 'ru' ? 'Опытный преподаватель' : 'Expert teacher') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                <div class="price-card sticky-sidebar">
                    <div class="price-header">
                        <span class="price-label">{{ app()->getLocale() == 'uz' ? 'Kurs narxi' : (app()->getLocale() == 'ru' ? 'Цена курса' : 'Course price') }}</span>
                        <div class="price-amount">
                            <span class="price">{{ number_format($course->price) }}</span>
                            <span class="currency">so'm</span>
                        </div>
                        <span class="price-period">{{ app()->getLocale() == 'uz' ? '/oyiga' : (app()->getLocale() == 'ru' ? '/месяц' : '/month') }}</span>
                    </div>
                    ...
                    <div class="price-details">
                        <div class="detail-row">
                            <span class="detail-label">{{ app()->getLocale() == 'uz' ? 'Tili' : (app()->getLocale() == 'ru' ? 'Язык' : 'Language') }}</span>
                            <span class="detail-value">{{ $course->language ?? (app()->getLocale() == 'uz' ? 'O\'zbek' : (app()->getLocale() == 'ru' ? 'Узбекский' : 'Uzbek')) }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">{{ app()->getLocale() == 'uz' ? 'Davomiylik' : (app()->getLocale() == 'ru' ? 'Длительность' : 'Duration') }}</span>
                            <span class="detail-value">{{ $course->duration ?? '—' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">{{ app()->getLocale() == 'uz' ? 'Joylar' : (app()->getLocale() == 'ru' ? 'Места' : 'Seats') }}</span>
                            <span class="detail-value success">{{ app()->getLocale() == 'uz' ? 'Mavjud ✓' : (app()->getLocale() == 'ru' ? 'Есть ✓' : 'Available ✓') }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
.header-icon.purple { background: linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%); }
.header-icon.green { background: linear-gradient(135deg, #10b981 0%, #34d399 100%); }
.header-icon.orange { background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%); }
...
function registerCourse(id, title) {
    ...
    Swal.fire({
        ...
        html: `
            ...
            <div style="text-align: left;">
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="text" id="name" class="swal2-input" style="margin: 0; width: 100%;" placeholder="${lang == 'uz' ? 'Ismingizni kiriting' : (lang == 'ru' ? 'Введите ваше имя' : 'Enter your name')}">
                </div>
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="tel" id="phone" class="swal2-input" style="margin: 0; width: 100%;" placeholder="+998 __ ___ __ __">
                </div>
                <div style="margin-bottom: 8px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #1e293b;">Email</label>
                    <input type="email" id="email" class="swal2-input" style="margin: 0; width: 100%;" placeholder="example@email.com">
                </div>
            </div>
        `,
        ...
        preConfirm: () => {
            ...
            if (!name || !phone || !email) {
                Swal.showValidationMessage(lang == 'uz' ? 'Barcha maydonlarni to\'ldiring' : (lang == 'ru' ? 'Заполните все поля' : 'Fill all fields'));
                return false;
            }
            if (!phone.match(/^\+998[0-9]{9}$/)) {
                Swal.showValidationMessage(lang == 'uz' ? 'Telefon: +998XXXXXXXXX formatida' : (lang == 'ru' ? 'Формат: +998XXXXXXXXX' : 'Format: +998XXXXXXXXX'));
                return false;
            }
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                text: lang == 'uz' ? 'Arizangiz qabul qilindi. Tez orada bog\'lanamiz' : (lang == 'ru' ? 'Заявка принята. Мы свяжемся с вами' : 'Application accepted. We will contact you'),
                timer: 3000,
